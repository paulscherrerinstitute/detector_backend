...
def get_c_det_submodule(detector_def, submodule_index):
    ...
    c_det_submodule.bytes_per_packet = detector_model.bytes_per_packet
    c_det_submodule.bytes_data_per_packet = detector_model.bytes_data_per_packet
...
def start_udp_receiver(udp_ip, udp_port, detector_def, submodule_index, ringbuffer, control_client):
    ...
    _logger.info("[%d] Starting udp_receiver submodule_index=%d with udp_ip='%s', udp_port=%d, " %
                 (submodule_index, submodule_index, udp_ip, udp_port))

    ringbuffer.init_buffer()

    _logger.debug("[%d] Ringbuffer initialized." % submodule_index)
    ...
    while True:
        ...
        _logger.debug("[%d] Starting udp_receive function." % submodule_index)
